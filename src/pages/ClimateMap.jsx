...
function ClimateMap() {
      ...
      case 'precipcomposite':
        return (
          <div className="space-y-2">
            ...
            <div className="h-2.5 w-full rounded-full bg-gradient-to-r from-sky-200 via-blue-200 via-indigo-200 to-purple-300" />
          </div>
        );
      case 'temp':
        return (
          <div className="space-y-2">
            ...
            <div className="h-2.5 w-full rounded-full bg-gradient-to-r from-cyan-200 via-teal-100 via-amber-100 via-orange-100 to-rose-300" />
          </div>
        );
      case 'wind':
        return (
      ...
      case 'cloudcover':
        return (
          <div className="space-y-2">
            ...
            <div className="h-2.5 w-full rounded-full bg-gradient-to-r from-slate-800 via-slate-500 to-slate-200" />
          </div>
        );
      default:
        return null;
  ...
  return (
    <div className="relative w-full h-[calc(100vh-5.25rem)] overflow-hidden bg-slate-950 font-sans flex flex-col lg:flex-row">
      ...
      <div className="absolute top-4 left-4 z-500 w-[calc(100%-2rem)] sm:w-[400px] max-h-[calc(100vh-8.5rem)] flex flex-col bg-slate-900/90 backdrop-blur-lg border border-slate-800/80 rounded-2xl shadow-2xl overflow-hidden transition-all duration-300">
        
        {/* Sidebar Header / Navigation */}
        <div className="flex border-b border-slate-800/60 p-2 bg-slate-950/40">
          <button
            onClick={() => setActiveTab('controls')}
            className={`flex-1 py-2 px-3 text-sm font-semibold rounded-lg transition-all duration-200 flex items-center justify-center gap-2 ${
              activeTab === 'controls' 
          ...
          {activeTab === 'controls' && (
            <>
              ...
              <form onSubmit={handleSearchSubmit} className="space-y-1">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Search Location</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl py-3 pl-11 pr-10 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200"
                  />
                ...
                {searchError && (
                  <div className="flex items-center gap-1.5 text-xs text-rose-500 mt-1 bg-rose-500/10 p-2 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {WEATHER_ELEMENTS.map((el) => (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-slate-500">Unit: {el.unit}</span>
                      {activeElement === el.id && (
                        <div className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${el.color}`} />
                      )}
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-mono text-emerald-400">{Math.round(opacity * 100)}%</span>
                </div>
                <input
                  type="range"
                  min="0.1"
                    ...
                    <button
                      ...
                      className={`py-2 px-1 text-center rounded-xl border text-xs font-semibold transition-all duration-200 cursor-pointer ${
                        baseMapType === key
                          ...
                          : 'bg-slate-950/40 border-slate-800/80 text-slate-400 hover:text-white hover:border-slate-700/85'
                      }`}
                    >
                      {mapConfig.name}
                    </button>
              ...
              <div className="text-xs text-slate-500 leading-relaxed bg-slate-950/30 border border-slate-800/20 rounded-xl p-3 flex gap-2">
            ...
            <div className="space-y-4">
              {weatherLoading && (
                <div className="flex flex-col items-center justify-center py-12 space-y-3">
                  ...
                  <span className="text-sm text-slate-400">Retrieving climate metrics...</span>
                </div>
              )}
              ...
              {!selectedCoords && !weatherLoading && (
                <div className="flex flex-col items-center justify-center text-center py-12 space-y-3 text-slate-400">
                  <Globe className="w-12 h-12 text-slate-600 stroke-1" />
                  <div>
                    <h4 className="font-semibold text-white text-sm">No Location Selected</h4>
              ...
              {weatherData && !weatherLoading && (
                <div className="space-y-4 animate-fadeIn">
                  ...
                  <div className="border-b border-slate-800 pb-3">
                    <div className="flex items-start justify-between gap-2">
                      <div className="space-y-0.5">
                        <h3 className="font-bold text-white text-base leading-snug text-wrap">
                  ...
                  <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-4 flex items-center justify-between">
                    <div className="space-y-1">
                      <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-xl font-medium text-emerald-400">°C</span>
                      </div>
                      <p className="text-xs text-slate-400">
                        Feels like: <span className="font-semibold text-slate-200">{Math.round(weatherData.currentConditions.feelslike)}°C</span>
                      </p>
                      <p className="text-sm font-semibold text-slate-100 capitalize mt-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {/* Humidity */}
                    <div className="bg-slate-950/40 border border-slate-800/50 rounded-xl p-3 flex items-center gap-3">
                      <div className="p-2 bg-emerald-500/10 text-emerald-400 rounded-lg">
                        <Droplets className="w-4 h-4" />
                      </div>
                      <div className="space-y-0.5">
                        <span className="text-[10px] uppercase font-bold text-slate-500">Humidity</span>
                        <p className="text-sm font-bold text-slate-200">{Math.round(weatherData.currentConditions.humidity)}%</p>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-950/40 border border-slate-800/50 rounded-xl p-3 flex items-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-sm font-bold text-slate-200">{Math.round(weatherData.currentConditions.windspeed)} km/h</p>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-950/40 border border-slate-800/50 rounded-xl p-3 flex items-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        <span className="text-[10px] uppercase font-bold text-slate-500">UV Index</span>
                        <p className="text-sm font-bold text-slate-200">{weatherData.currentConditions.uvindex} / 10</p>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-950/40 border border-slate-800/50 rounded-xl p-3 flex items-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        <span className="text-[10px] uppercase font-bold text-slate-500">Clouds</span>
                        <p className="text-sm font-bold text-slate-200">{Math.round(weatherData.currentConditions.cloudcover)}%</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-950/60 border border-slate-800/50 rounded-xl p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs">
                      <div className="space-y-1">
                        <span className="text-slate-500 font-medium">Solar Radiation</span>
                        <p className="font-bold text-slate-200">{weatherData.currentConditions.solarradiation ? `${Math.round(weatherData.currentConditions.solarradiation)} W/m²` : 'N/A'}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-slate-500 font-medium">Solar Energy</span>
                        <p className="font-bold text-slate-200">{weatherData.currentConditions.solarenergy ? `${weatherData.currentConditions.solarenergy} MJ/m²` : 'N/A'}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-slate-500 font-medium">Atmospheric Pressure</span>
                        <p className="font-bold text-slate-200">{weatherData.currentConditions.pressure ? `${Math.round(weatherData.currentConditions.pressure)} hPa` : 'N/A'}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-slate-500 font-medium">Timezone</span>
                        <p className="font-bold text-slate-200 truncate">{weatherData.timezone}</p>
                      </div>
                    </div>
                  </div>

                  {/* Sun Times */}
                  <div className="bg-slate-950/40 border border-slate-800/30 rounded-xl p-3.5 flex justify-between text-xs text-slate-400">
                    <div className="flex items-center gap-2">
                      <Compass className="w-4 h-4 text-amber-500" />
                      <div>
                        <span className="text-[10px] text-slate-500 block">SUNRISE</span>
                        <span className="font-bold text-slate-300">{weatherData.currentConditions.sunrise.substring(0, 5)} AM</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <span className="font-bold text-slate-300">{weatherData.currentConditions.sunset.substring(0, 5)} PM</span>
                      </div>
                    </div>
                  </div>
        ...
        {/* Footnote */}
        <div className="bg-slate-950/80 px-5 py-3 border-t border-slate-800/80 flex items-center justify-between text-[10px] text-slate-500">
